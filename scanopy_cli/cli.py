...
@click.option("--network-id", "-i", required=True, help="Network ID.")
@click.option("--view", "-v", default="L3Logical", help="View name (default: L3Logical).")
@click.option("--format", "-f", "fmt", type=click.Choice(["mermaid", "json", "png", "pdf", "html"]), default="png", help="Export format.")
@click.option("--output", "-o", "outfile", default="", help="Output file path.")
@click.option("--width", default=2400, help="Image width (default: 2400).")
@click.option("--height", default=1600, help="Image height (default: 1600).")
@click.pass_context
def topology_export_cmd(ctx: click.Context, network_id: str, view: str, fmt: str, outfile: str, width: int, height: int) -> None:
    """Export topology as PNG, PDF, HTML, Mermaid, or JSON.

    PNG/PDF use vis.js + Playwright headless Chromium for proper graph layout.
    HTML generates a standalone interactive file.

    \b
    Examples:
      sc topology-export -i NET_ID -f png -o network.png
      sc topology-export -i NET_ID -f pdf -o network.pdf
      sc topology-export -i NET_ID -f html -o network.html
      sc topology-export -i NET_ID -f mermaid
      sc topology-export -i NET_ID -f json
    """
    try:
        envelope = ctx.obj["client"].topology_data(network_id)
        ...
        if view and isinstance(payload, dict):
            ...
            if available and view not in [str(v) for v in available]:
                if console:
        ...
        if fmt == "mermaid":
            from .server_mermaid import fetch_server_mermaid
            topo_id = payload.get("id") if isinstance(payload, dict) else None
            if topo_id:
                output = fetch_server_mermaid(ctx.obj["client"], topo_id, view)
            else:
        ...
        # Visual formats — vis.js + Playwright
        from .renderer import render_topology_png, render_topology_pdf, render_topology_html

        if not outfile:
            outfile = f"topology.{fmt}"

        if fmt == "png":
            msg = render_topology_png(envelope, outfile, view=view, width=width, height=height)
        elif fmt == "pdf":
            msg = render_topology_pdf(envelope, outfile, view=view, width=width, height=height)
        elif fmt == "html":
            msg = render_topology_html(envelope, outfile, view=view)
